...
def uas_infer(uas):
    ...
    checkpoint_path = "/root/autodl-tmp/graph_model/Diffusion_directed_evolution/oracle_ckpt/uas_hybridnet_oracle.ckpt"
    ...
    uas_strength = model(uas)
    
    return uas_strength


@torch.no_grad()
def cp_infer(core_promoter):
    model = HybridNet().to(device)
    checkpoint_path = "/root/autodl-tmp/graph_model/Diffusion_directed_evolution/oracle_ckpt/corepromoter_hybridnet_oracle.ckpt"
    checkpoint = torch.load(checkpoint_path, map_location=device)
    if 'state_dict' in checkpoint:
        state_dict = {k.replace('model.', ''): v for k, v in checkpoint['state_dict'].items()}
        model.load_state_dict(state_dict)
    ...
    cp_strength = model(core_promoter)
    
    return cp_strength

@torch.no_grad()
def promoter_500bp_infer(core_promoter):
    model = HybridNet().to(device)
    checkpoint_path = "/root/autodl-tmp/graph_model/Diffusion_directed_evolution/seqs_and_embeddings/inference_dataset/ckpt/OneHot_Promoter_Gene_Genera.ckpt"
    checkpoint = torch.load(checkpoint_path, map_location=device)
    if 'state_dict' in checkpoint:
        state_dict = {k.replace('model.', ''): v for k, v in checkpoint['state_dict'].items()}
        model.load_state_dict(state_dict)
...
# light attention finetune, model code
import torch
...
def kmers_stride1(seq: str, k: int = 6):
...
@torch.no_grad()
def specieslm_sc_infer(seqs, device='cuda:0'):
    model = SpeciesLMLightAttention().to(device)
    ckpt = torch.load("/root/autodl-tmp/graph_model/Diffusion_directed_evolution/SpeciesLM/finetune_ckpts/sc_ft_last2layers.ckpt", map_location="cuda:0")
    ...
    batch_tok = tokenizer(['kazachstania_africana_cbs_2517_gca_000304475' + " " + " ".join(kmers_stride1(s)) for s in seqs],
        padding=True,
        truncation=True,
        return_tensors="pt"
    ).to(device)
    ...
    model.eval()
    preds = model(batch_tok["input_ids"], batch_tok["attention_mask"])
