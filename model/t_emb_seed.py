...
class PromoterModel(nn.Module):
    ...
    def __init__(self, args, embed_dim=256, time_dependent_weights=None, time_step=0.01):
        ...
        self.seed_proj_layer = nn.Linear(2560, embed_dim)
        expanded_simplex_input = (args.mode == 'dirichlet' or args.mode == 'riemannian')
        inp_size = self.alphabet_size * (2 if expanded_simplex_input else 1) # plus one for signal input
    ...
    def forward(self, x, seed, t):
        # Obtain the Gaussian random feature embedding for t
        # embed: [N, embed_dim]
        t_embed = self.act(self.embed(t / 2))
        #seed = seed.view(seed.shape[0], 256, -1)
        seed_proj = self.seed_proj_layer(seed)
        t_embed = t_embed + seed_proj
        ...
        #emb_concat = torch.cat([x, emb_reshaped], dim=1)
